...
export default function SandboxBuilder({
  ...
  // ── New project flow ──────────────────────────────────────────────
  if (id === "new" && !project) {
    return <NewProjectForm onCreate={(p) => { navigate(`/sandbox/${p.id}`); }} />;
  }
        ...
        <div className="flex items-center gap-2">
          {project && stage !== "idea" && (
            <>
              ...
              <button onClick={() => navigate("/sandbox")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#64748b] hover:text-red-600 rounded-lg transition">
