...
import { grokChatJSON } from "../lib/sandbox/grok.js";
import {
  extractMetadataFromDoiOrTitle,
  extractMetadataFromPdfText,
  extractAuthorsFromPdfText,
  fetchCrossrefPagesByDoi,
  extractPageRangeFromOjsHtml,
} from "../lib/citation/metadataFetcher.js";
...
function looksLikeDomain(s: string): boolean {
  ...
  if (/\s(newspaper|magazine|news|blog|website|site|portal|hub|online|daily|weekly|monthly|times|post|herald|tribune|chronicle|journal)$/i.test(lower)) return false;
...
router.post("/citation/scrape", async (req, res) => {
  try {
    const { url } = req.body;
    const userTitle: string | undefined = req.body.title;
    ...
    try {
      ...
      return res.status(400).json({ error: "Invalid URL format. Please enter a full URL like https://example.com/article" });
    }
    ...
    try {
      const resPage = await fetch(cleanUrl, {
        headers: {
      ...
      if (resPage.ok) {
        const contentType = resPage.headers.get("content-type") || "";
        if (contentType.includes("pdf")) {
          const pdfBuffer = await resPage.arrayBuffer();
          if (pdfBuffer && pdfBuffer.byteLength > 100) {
            const pdfResult = await parsePdfFromBuffer(Buffer.from(pdfBuffer), parsedUrl.pathname);
            return res.json({
              title: pdfResult.title || "",
    ...
    let siteName = "";
    let authors: string[] = [...pdfBylineAuthors];
    let pubDate = "";
    let volume = "";
    let issue = "";
    let pages = "";
    ...
    // Step 4b: If OAI-PMH gave only a single page number, try direct Crossref DOI lookup
    ...
    // Step 4c: Last-resort fallback for OJS page ranges
    ...
    if (authors.length === 0 && /cran/i.test(hostname)) {
      ...
      if (pkg) {
        try {
          ...
          // CRAN DESCRIPTION fetch failed
        }
      }
    }

    // Step 5: Run LLM only for fields still missing
    let fallbackDate = "";
...
${snippet}`;
    ...
    // Step 5b: When no HTML was available and we're still missing essential data, try LLM with URL context
...
- If you don't know any field, use empty string (not null, not "null")`;
    ...
    return res.status(500).json({ error: "Could not scrape metadata. Please fill in details manually." });
  }
});
...
router.post("/citation/extract-pdf", upload.single("file"), async (req, res) => {
  try {
    ...
    // Step 2: Use Crossref data as the baseline, then fill remaining gaps via LLM
...
${snippet}`;
  ...
  } catch (e) { return res.status(500).json({ error: e instanceof Error ? e.message : "Failed to process PDF" }); }
});
